    ...
    with open(_fds_file) as f:
        ...
        for l in fds_lines:
            if l.startswith("&MESH"):
                l_split = l.split('IJK=')[-1]
                l_resplit = re.split(r'[,=/]+', l_split)
                l_strip = [x.strip() for x in l_resplit]
                for elem in ['XB', 'MPI_PROCESS', '\n']:
                    if l_strip.count(elem):
                        l_strip.remove(elem)
                l_values = np.array(l_strip[:9], dtype=float)
                x_dims.extend(l_values[3:5])
                y_dims.extend(l_values[5:7])
                z_dims.extend(l_values[7:9])
                _dx = (l_values[4] - l_values[3]) / l_values[0]
                _dy = (l_values[6] - l_values[5]) / l_values[1]
                _dz = (l_values[8] - l_values[7]) / l_values[2]
...
def plot_line_sights(_Time, _dx, _dy, _dz, _point_of_view, _agent_exit):
    ...
    p_file = os.path.join(sfgrids_path, '%s/Z_2.250000/Door_X_%.6f_Y_%.6f/t_%.f.000000.npz'
                          % (quantity, x_exit, y_exit, _Time))
    sfgrid = np.load(p_file)['smoke_factor_grid_norm']
    x0 = x_0 / _dx
    y0 = y_0 / _dy
    x, y = m_to_pix(x_i=exits[_exit][0], y_i=exits[_exit][1])
    x_exit, y_exit = np.linspace(x0, x, np.hypot(x - x0, y - y0)), np.linspace(y0, y, np.hypot(x - x0, y - y0))
...
def main():
    global fds_path, logfile, sfgrids_path, fds_file, quantity, specified_location, \
        x_min, x_max, y_min, y_max, dx, dy, dim_x, dim_y, exits, delta_smoke_factor_grid, convert, Time
    # Parse parameters
    ...
    for iis in sids:
        ...
        slice_data.readAllTimes(root_dir)
        # read in slice data
        slice_data.readTimeSelection(root_dir, dt=t_step, average_dt=1)
        # map data on mesh
        slice_data.mapData(meshes)
    ...
    if plots:
        ...
        for _id, it in enumerate(slice_data.times):
            ...
            plt.colorbar(im, cax=cax, label="{} [{}]".format(slice_data.quantity, slice_data.units))
