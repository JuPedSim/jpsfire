...
def main(convert, exit):
    ...
    if not os.path.exists('../%s/sfgrids'%chid):
        print '\ncreate directory "sfgrids"\n'
        os.makedirs('../%s/sfgrids'%chid)

    if not os.path.exists('../%s/sfgrids/z_%s'%(chid, clip_dim3)):
        print '\ncreate directory "z_*"\n'
        os.makedirs('../%s/sfgrids/z_%s'%(chid, clip_dim3))
        
    if not os.path.exists('../%s/sfgrids/z_%s/dx_%.2f'%(chid, clip_dim3, delta_mesh_exit)):
        print '\ncreate directory "sfgrids/z_%s/dx_%.2f"\n'
        os.makedirs('../%s/sfgrids/z_%s/dx_%.2f'%(chid, clip_dim3, delta_mesh_exit))
        
    if not os.path.exists('../%s/sfgrids/z_%s/dx_%.2f/Door_X_%.6f_Y_%.6f'%(chid, clip_dim3, delta_mesh_exit, exits[exit][0], exits[exit][1])):
        os.makedirs('../%s/sfgrids/z_%s/dx_%.2f/Door_X_%.6f_Y_%.6f'%(chid, clip_dim3, delta_mesh_exit, exits[exit][0], exits[exit][1]))
    ...
    time=float(convert[:-4][convert.find('_')+1:])
    ...
    np.savetxt('../%s/sfgrids/z_%s/dx_%.2f/Door_X_%.6f_Y_%.6f/t_%6f.csv'\
    %(chid, clip_dim3, delta_mesh_exit, exits[exit][0], exits[exit][1], time), mesh_exit, header=header, delimiter=',', comments='')
    print 'Write smoke factor grid: ../%s/sfgrids/z_%s/dx_%.2f/Door_X_%.6f_Y_%.6f/t_%6f.csv'\
    %(chid, clip_dim3, delta_mesh_exit, exits[exit][0], exits[exit][1], time)
...
clip_dim3s = [2.0, 2.2, 2.4, 2.6, 2.8, 3.0][4:5]
delta_mesh_exits = [4.0, 2.0, 1.0, 0.5, 0.25][2:3]
...
for clip_dim3 in clip_dim3s:
    ...
    print '\n-----------------------------------------------------------------'
    print 'Analysing Slice File at z=%s\n' %clip_dim3


    for delta_mesh_exit in delta_mesh_exits:

        #plt.close()
        converted = glob.glob('../%s/converted/%s/SOOT_165.csv'%(chid, clip_dim3))
        ...
        for convert in converted:
            ...
            for id, exit in enumerate(exits):
                ...
                a,b, x0, y0, x, y, zi_exit, edge_exit, x_shift, y_shift = main(convert, exit)
            ...
            plt.savefig('../%s/converted/%s/smoke%s.pdf'%(chid, clip_dim3, convert[:-4][convert.find('_'):]))
