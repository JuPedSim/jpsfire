...
#===============================================================#
                                                                #
clip_dim3=29.         #Dimension 3 zum Schnitt der Geometrie    #
                                                                #
#===============================================================#


chid = 'V2_1_summer_dx15'

if not os.path.exists('../converted/%s'%clip_dim3):
    print 'create directory "converted"'
    os.makedirs('../converted/%s'%clip_dim3)
if not os.path.exists('../slicefiles/%s'%clip_dim3):
    print 'create directory "slicefiles"'
    os.makedirs('../slicefiles/%s'%clip_dim3)


fds=glob.glob('../*.fds'%chid)
#===== if necessary, adjust path and/or name of the converted asci slices======
slicefiles=glob.glob('../ascii/%s/*'%clip_dim3)
...
#==========extraction of &OBST + &HOLE lines ==================================
    ...
    with open(fds[0]) as f:
        ...
        if slicefile == slicefiles[0]:
            ...
            np.savetxt('../converted/obst.csv', obsts, delimiter=',')
        ...
        if slicefile == slicefiles[0]:
            ...
            np.savetxt('../converted/hole.csv', holes, delimiter=',')

    collect = magnitudes + geometry
    #collect = collect[:-1,:-1]

    np.savetxt('../converted/%s/%s.csv'%(clip_dim3, slicefile[slicefile.rfind('/')+1:-4]), collect, delimiter=',')
    print '\nWrite files: ../converted/%s/%s.csv'%(clip_dim3, slicefile[slicefile.rfind('/')+1:-4])
...
#        plt.contourf(dim1, dim2, collect)
    ...
    plt.savefig('../slicefiles/%s/slice%s.pdf'%(clip_dim3, slicefile[slicefile.rfind('/')+1:-4]))
    #plt.show()
